...
class System(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        #self.log("lr", cur_lr, prog_bar=True, on_step=True, logger=True)
        #self.log("wd", cur_wd, prog_bar=True, on_step=True, logger=True)

        self.log("loss", loss, logger=True)
        self.log("train_loss", loss, logger=True, )

        self.logger.experiment.add_scalars("losses", {"train": loss},
    ...
    def validation_step(self, batch, batch_nb):
        ...
        self.logger.experiment.add_scalars(
            "losses", {"val": loss}, global_step=self.global_step)
    ...
    def on_validation_epoch_end(self):
        ...
        if hp_metric is not None:
            self.trainer.logger.log_metrics({"hp_metric_val_loss": hp_metric}, step=self.trainer.global_step)
        ...
        # Force print to stdout, from https://stackoverflow.com/questions/57759784/how-to-force-python-to-print-when-called
        ...
        for idx in range(len(self.batch_example)):
          # Set the example
          example_input, example_target = self.batch_example[idx]
          #print(f"\nThe shape of the example input is {example_input.shape}")
          #print(f"The shape of the example target is {example_target.shape}")
          
          num_sources = example_target.shape[0]

          # On every validation end
          predictions = self(example_input.to(self.device)).cpu().data.numpy()
          
          example_input = example_input.cpu().data.numpy()
          example_target = example_target.cpu().data.numpy()
          predictions = normalize_estimates(predictions, example_input)

          #print(f"The shape of the predictions is {predictions.shape}\n")
          
          if self.global_step == 0:
            self.logger.experiment.add_audio(f"input_{idx}_{self.global_step}",
                                        example_input,
                                        global_step=0,
          ...
          for s_idx in range(num_sources):
            ...
            if self.global_step == 0:
              self.logger.experiment.add_audio(f"source_{idx}_{s_idx+1}_{self.global_step}",
                                            example_target[s_idx],
                                            global_step=0,
                                            sample_rate=self.sample_rate)
              self.logger.experiment.flush()

            self.logger.experiment.add_audio(f"estimated_source_{idx}_{s_idx+1}_{self.global_step}",
                                          predictions[s_idx],
                                          global_step=self.global_step,
                                          sample_rate=self.sample_rate)
            self.logger.experiment.flush()
