...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the essid username if it exists
    if id essid &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER essid
    fi
    ...
    if [[ "$NB_USER" != "essid" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/essid "/home/$NB_USER" || ln -s /home/essid "/home/$NB_USER"
        fi
        # if workdir is in /home/essid, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/essid/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u essid)" && "$NB_GID" == "$(id -g essid)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^essid:/nayvoj:/" > /tmp/passwd
                echo "essid:x:$(id -u):$(id -g):,,,:/home/essid:/bin/bash" >> /tmp/passwd
